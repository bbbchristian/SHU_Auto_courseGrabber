...
- **自动展开课程信息**
  - 由于在选课的时候需要页面上展现出所有需要的信息，所以过滤器生效的条件就是课程信息卡片是展开状态。但是因为 jQuery 后DOM树自动重塑会关闭卡片，所以本脚本实现了刷新后自动 click 事件展开目标卡片
  - 但是由于展开会带来额外的性能消耗、抢课延迟，所以当用户是以下情境时会自动将 CLICK2EXPEND_ENABELD 设置为 false ：
    1. 页面上只有一门课且是目标课程（因为正方选课系统会自动在刷新之后展开页面的第一门课）
    2. 不需要时间过滤或者教师过滤（其实没有过滤目标在调用展开函数后也会立马结束，但是这点延迟也可能会影响发送抢课请求的速率！）
