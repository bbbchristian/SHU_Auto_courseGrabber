...
- **自动展开课程信息** -
  - 由于在选课的时候需要页面上展现出所有需要的信息，所以过滤器生效的条件就是课程信息卡片是展开状态。但是由于 jQuery 后DOM树自动重塑会关闭其，所以本脚本实现了自动 click 事件展开目标卡片
  - 并且由于展开会带来额外的性能消耗、抢课延迟，所以当用户是以下情境时会自动将 **CLICK2EXPEND_ENABELD** 设置为 false ：
