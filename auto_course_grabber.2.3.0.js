...
(function() {
    ...
    const TARGET_START_TIME = '';    // 填入开抢时间 (格式: YYYY-MM-DD HH:mm:ss)
    ...
    let isRunning = false;
    let isCountingDown = false;             // 倒计时状态标志
    let intervalId = null;
    ...
    function log(message, type = 'info') {
        ...
        switch(type) {
            ...
            case 'info':
                ...
                break;
            case 'timer':
                console.log(`%c${prefix} ⏱️ ${message}`, 'color: #9370db; font-weight: bold;');
                break;
        }
    }
...
function findAllTeachingClasses() {
    ...
    function startCountdown(targetTime) {
        if (isCountingDown) {
            log('倒计时已在运行中！', 'warning');
            return;
        }
        
        const targetDate = new Date(targetTime);
        if (isNaN(targetDate.getTime())) {
            log('无效的时间格式！请使用 YYYY-MM-DD HH:mm:ss 格式', 'error');
            return;
        }
        ...
        function updateCountdown() {
            ...
            // 显示倒计时
            log(`⏱️ 距离抢课开始还有: ${hours}小时 ${minutes}分钟 ${seconds}秒`, 'timer');
        }
    ...
    // 获取状态
    function getStatus() {
        let countdownInfo = "未设置倒计时";
        if (isCountingDown && TARGET_START_TIME) {
            const targetDate = new Date(TARGET_START_TIME);
            const now = new Date();
            const diff = targetDate - now;
            ...
            if (diff > 0) {
                ...
                const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
                const seconds = Math.floor((diff % (1000 * 60)) / 1000);
                countdownInfo = `${hours}小时 ${minutes}分钟 ${seconds}秒`;
            } else {
                countdownInfo = "倒计时已结束";
            }
        }
        ...
        const status = {
            ...
            targetStartTime: TARGET_START_TIME || "未设置",
    ...
    window.courseGrabber = {
        start: function() {
            if (TARGET_START_TIME) {
                log(`⏰ 检测到定时抢课设置: ${TARGET_START_TIME}`, 'timer');
                startCountdown(TARGET_START_TIME);
            } else {
                startGrabbing();
            }
        },
    ...
    if (TARGET_START_TIME) {
        ...
        if (!isNaN(targetDate.getTime())) {
            ...
            if (targetDate > now) {
                log(`⏰ 检测到定时抢课设置: ${TARGET_START_TIME}`, 'timer');
                log('输入 courseGrabber.start() 启动倒计时', 'info');
            } else {
                log(`⚠️ 定时时间 ${TARGET_START_TIME} 已过期，请更新配置`, 'warning');
            }
        } else {
            log(`❌ 无效的定时时间格式: ${TARGET_START_TIME}`, 'error');
        }
    }
    
    // 显示脚本信息
    console.log('%c🎓 上海大学自动抢课脚本已加载', 'color: #ff6b35; font-size: 18px; font-weight: bold;');
    console.log('%c📚 目标课程: ' + (TARGET_COURSE_CODE || '未设置'), 'color: #4ecdc4; font-size: 14px; font-weight: bold;');
    console.log('%c⏰ 定时开抢: ' + (TARGET_START_TIME || '未设置'), 'color: #9370db; font-size: 14px; font-weight: bold;');
    console.log('%c⚡ 使用方法:', 'color: #45b7d1; font-size: 14px; font-weight: bold;');
    console.log('  courseGrabber.start()  - 🚀 开始抢课/倒计时');
    console.log('  courseGrabber.stop()   - ⏹️ 停止抢课/倒计时');
